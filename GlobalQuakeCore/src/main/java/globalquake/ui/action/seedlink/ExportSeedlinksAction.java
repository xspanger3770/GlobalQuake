...
public class ExportSeedlinksAction extends AbstractAction {
    ...
    public ExportSeedlinksAction(Window parent, StationDatabaseManager databaseManager) {
        super("Export");
    ...
    private java.util.List<String[]> createData() {
        ...
        result.add(new String[]{"Name", "Host", "Port", "Timeout (s)"});
        for (SeedlinkNetwork seedlinkNetwork : databaseManager.getStationDatabase().getSeedlinkNetworks()) {
            result.add(new String[]{
                    seedlinkNetwork.getName().replace('"', ' '),
                    seedlinkNetwork.getHost().replace('"', ' '),
                    String.valueOf(seedlinkNetwork.getPort()),
                    String.valueOf(seedlinkNetwork.getTimeout())});
        }
        return result;
    }
