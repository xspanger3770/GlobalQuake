...
public class EarthquakeSimulatorPanelOld extends GlobePanelOld {
    ...
    protected double[] selectStuff() {
        ...
        JSlider slider2 = createDepthSlider();
    ...
    static JSlider createDepthSlider() {
    ...
    public void paint(Graphics gr) {
        ...
        synchronized (simulator.earthquakesSync) {
            for (SimulatedEarthquake e : simulator.getEarthquakes()) {
        ...
        for (ArchivedQuake quake : simulator.getFakeGlobalQuake().getArchive().getArchivedQuakes()) {
            ...
            double ageInHRS = (System.currentTimeMillis() - quake.getOrigin()) / (double) (1000 * 60 * 60);
            ...
            if (mouseNearby && scroll < 5) {
                ...
                if (quake.getArchivedEvents() != null) {
                    for (ArchivedEvent event : quake.getArchivedEvents()) {
                        ...
                        g.setColor(event.abandoned() ? Color.orange : Color.green);
        ...
        for (Cluster c : simulator.getClusterAnalysis().getClusters()) {
            ...
            if (c.bestAngle >= 0) {
                ...
                double[] values = GeoUtils.moveOnGlobe(c.getRootLat(), c.getRootLon(), 200, c.bestAngle);
                double lat1 = values[0];
                double lon1 = values[1];
